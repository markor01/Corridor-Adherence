...
% with = load('simdata_CBF_with.mat');   % loads simdata and t
% without = load('simdata_CBF_without.mat');   % loads simdata and t

% with = load('simdata_CBF_with_tighter.mat');   % loads simdata and t
% without = load('simdata_CBF_without_tighter.mat');   % loads simdata and t

with = load('simdata_CBF_disturbance_with.mat');   % loads simdata and t
without = load('simdata_CBF_disturbance_without.mat');   % loads simdata and t
...
e_y_max_without     = without.simdata(:,20);                 % m
dx_p_without        = without.simdata(:,21);
dy_p_without        = without.simdata(:,22);
e_y_without         = without.simdata(:,23);                 % m
...
e_y_max_with     = with.simdata(:,20);                 % m
dx_p_with        = with.simdata(:,21);
dy_p_with        = with.simdata(:,22);
e_y_with         = with.simdata(:,23);                 % m
...
plot(y_with, x_with, 'b-', 'LineWidth',1.5, 'DisplayName','With CBF')
plot(y_without, x_without, '-', 'LineWidth',1.5, 'DisplayName','Without CBF')
...
plot(t_with, e_y_with, 'b', 'LineWidth', 1.5, 'DisplayName', 'Cross-track error y_e with CBF');
plot(t_without, e_y_without, '-', 'LineWidth', 1.5, 'DisplayName', 'Cross-track error y_e without CBF');
...
title('\psi_{d,safe} with vs without CBF'); xlabel('Time (s)'); ylabel('Angle (deg)');
legend('\psi_{d,safe} with CBF', '\psi_{d,safe} without CBF')
grid on;
subplot(212)
plot(t_with,delta_c_deg_with, t_without,delta_c_deg_without, '-','linewidth',2);
title('Commanded rudder angle with and without CBF'); xlabel('Time (s)'); ylabel('Angle (deg)');
legend('\delta_c with CBF','\delta_c without CBF')
grid on;