classdef Navigator < handle % handle makes it behave like python objects
    ...
    methods
        ...
        function [x_p, y_p, dx_p, dy_p, e_y_max, U_max, kappa] = getPathReference(obj, path_var)
            ...
            [x_p, y_p, dx_p, dy_p, ~, seg_idx, kappa] = pathParam(path_var, obj.path);
        ...
        function R_seg = getCurrentRadius(obj, path_var)
            % Returns the turning radius of the current segment
        ...
        function seg_idx = getSegIdx(obj, path_var)
            [~,~,~,~,~,seg_idx] = pathParam(path_var, obj.path);
        end
    end
